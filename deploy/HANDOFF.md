# ContentForge — VPS Build Handoff

The build agent is already running on the Netcup VPS. This doc is about driving the build, not setup. The plan and stack live in PLAN.md + docs/hld.md + docs/lld.md; progress lives on board #10.

## Quick reference

| Thing | Where |
|---|---|
| Agent UI | http://localhost:3000 (or your Cloudflare Tunnel URL) |
...
## Kickoff (one message in a new conversation)

Open the contentforge workspace in the agent and send:

```text
Start P0.
```

The brief loads automatically and drives the build: reads the docs + board, executes P0 (monorepo merge of the two source systems into the uv workspace), verifies acceptance criteria, updates the board, commits, pushes — then proceeds P1, P2, ... in order. It stops and posts to the issue before any architecture change.

If the brief does not auto-load in your version, paste the contents of `.openhands/microagents/repo.md` as the first message instead.
...
- **RAM budget** (8 GB box): the agent ~1-2 GB + one render at a time (3-4 GB). Never run parallel renders.
...
| Problem | Fix |
|---|---|
| Agent can't find source repos | They must be inside the workspace: `~/contentforge/_sources/` (the brief clones them at P0 start; private repos need GITHUB_TOKEN) |
...
| Board not updating | gh missing or GITHUB_TOKEN not reaching the sandbox — agent falls back to the GitHub GraphQL API / issue posts; ensure GITHUB_TOKEN is set on the host env |
| First commit fails with "tell me who you are" | The brief sets the owner's git identity repo-locally before the first commit |
| Agent container down | `docker ps -a` to find it, then `docker start <name>` (host) |
