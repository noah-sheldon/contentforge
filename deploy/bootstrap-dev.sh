...
echo "    3. cd ~/contentforge      # set .env keys (DEEPSEEK_API_KEY etc.), export GITHUB_TOKEN"
echo "    4. ./deploy/openhands-run.sh   # start OpenHands (agent; brief auto-loaded from .openhands/microagents)"
echo "       alternative: tmux new -s dev && qwen   # Qwen Code CLI agent"
echo ""
echo "    Recommend: SSH key auth for root + dev user (PasswordAuthentication no)."
echo "    VPS specs assumed: Netcup VPS 1000 G12 (4 vCPU / 8 GB) — render queue concurrency 1."
