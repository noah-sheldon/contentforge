  ...
  const walk = (dir) => {
    ...
    for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) walk(full);
        ...
        found.push({
          name: rel,
          path: `/read?path=${encodeURIComponent(rel)}`,
          label: labelFor(rel),
        });
      }
    }
...
function buildIndex(scripts) {
  const rows = scripts
    .map((s) => {
      ...
      return (
        `<a class="row" href="/prompter?path=${pathParam}"><span class="label">${s.label}</span>` +
...
app.get("/api/scripts", (_req, res) => res.json(collectScripts()));
...
app.get("/prompter", (_req, res) => res.sendFile(PROMPTER));

app.get("/", (_req, res) => res.type("html").send(buildIndex(collectScripts())));
