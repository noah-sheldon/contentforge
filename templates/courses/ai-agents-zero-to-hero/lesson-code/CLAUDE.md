# HyperFrames Composition Project — lesson-code
...
Always invoke the relevant skill before writing or modifying compositions: `/hyperframes` (router), `/hyperframes-core` (composition contract), `/hyperframes-animation` (motion), `/hyperframes-keyframes` (typewriter/cursor effects). Skipping them produces broken compositions.
...
5. Only deterministic logic — no `Date.now()`, no `Math.random()`, no `repeat: -1` (finite cursor blinks only), no network at render time.
6. No CSS initial `transform` on any element that GSAP also tweens — use `fromTo` for initial states.
7. Code lines keep `white-space: pre` and stay inside `--pad` margins — never a full-bleed editor (black-screen failure mode).
