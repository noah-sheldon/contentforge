# HyperFrames Composition Project — lesson-whiteboard
...
Always invoke the relevant skill before writing or modifying compositions: `/hyperframes` (router), `/hyperframes-core` (composition contract), `/hyperframes-animation` (motion), `/hyperframes-creative` (design/typography). Skipping them produces broken compositions.
...
4. Scene fills go on full-bleed children (`position:absolute; inset:0`), never on the root.
5. Only deterministic logic — no `Date.now()`, no `Math.random()`, no `repeat: -1`, no network at render time.
6. No CSS initial `transform` on any element that GSAP also tweens — use `fromTo` for initial states.
7. Text elements are block-level + sized; no `<br>` in body text.
8. Every reveal is speech-timed: `at` values come from the VO transcript mapping, never a fixed stage formula.
